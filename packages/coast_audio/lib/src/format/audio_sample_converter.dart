...
abstract class AudioSampleConverter {
  ...
  void convert(Uint8List inputBuffer, Uint8List outputBuffer, [int inputOffset = 0, int outputOffset = 0]);

  /// Converts all samples from [inputBuffer] and writes the result to [outputBuffer].
  void convertAll(Uint8List inputBuffer, Uint8List outputBuffer, [int inputOffset = 0, int outputOffset = 0]) {
    final sampleCount = inputBuffer.length ~/ inputBytes;
    for (var i = 0; sampleCount > i; i++) {
      convert(inputBuffer, outputBuffer, inputOffset, outputOffset);
